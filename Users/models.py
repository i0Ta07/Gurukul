...
def generate_access_code(prefix, length=6):
    ...
    uid = shortuuid.uuid().int  # Small unique integer
    code = to_base36(uid)[-length:].rjust(length, '0')  # Pick last `length` chars
...
class Attempt(models.Model):
    ...
    question_start_timestamp = models.DateTimeField(
        verbose_name=_('start question timestmap'),
        help_text=_('When the start question ended'),
    )
