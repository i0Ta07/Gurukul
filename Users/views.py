...
def start_quiz(request, quiz_id):
    ...
    if attempt:
        ...
        # **Deduct Time on Every Request (Both GET & POST)**
        if attempt.question_start_timestamp:
            time_spent = (now() - attempt.question_start_timestamp).total_seconds()
            attempt.remaining_time = max(0, attempt.remaining_time - time_spent)
            attempt.save(update_fields=['remaining_time'])


        # If time is up, auto-submit
        if attempt.remaining_time <= 0:
            complete_quiz(attempt,status="timeout")
            return redirect("quiz_submission", attempt.attempt_id)
    ...
    # *********** If No Attempt Exists (New Quiz) ***********
        ...
        attempt = Attempt.objects.create(
            ...
            question_start_timestamp=now(),
            marks_scored= 0,
            current_question_index = 0
        )
        attempt.shuffle_questions()
        
    current_question = attempt.get_current_question(attempt.current_question_index) # cannot combine and write down after if-else
    current_question.shuffle_options()
   
    # ** Handle Form Submission Before Changing Question Index**
    if request.method == 'POST':
        form = SingleQuestionForm(request.POST, quiz=quiz, user=request.user, current_question=current_question)
        if form.is_valid():
            answer_value = form.cleaned_data['answer']
                # Trim whitespace only for text-based answers
            if isinstance(answer_value, str):
                answer_value = answer_value.strip()
            
            
            if answer_value == '' and "marked_for_review" not in attempt.question_status.get(str(current_question.question_id), ""):
                attempt.question_status[str(current_question.question_id)] = "unanswered"
            ...
            if answer_value!='':
                Answer.objects.update_or_create(
                    ...
                    quiz=quiz,
                    attempt=attempt,
                    defaults={'text_answer': answer_value if current_question.question_type == 'text' else None,
                              'selected_option': answer_value if current_question.question_type in ['mcq', 'true_false'] else None}
                )
                
                if "marked_for_review" not in attempt.question_status.get(str(current_question.question_id), ""):
                    attempt.question_status[str(current_question.question_id)] = "answered"
                
                
            # **Process Navigation: Move to Next/Previous/Jump**
            jump_to_question = request.POST.get("jump_to_question")
            go_back = request.POST.get("previous_question")
            go_forward = request.POST.get("next_question")
            clear_response = request.POST.get("clear_response")
            confirm_submit = request.POST.get('confirm_submit')
            marked_for_review = request.POST.get('marked_for_review')

            # mark for review and save is only confirm submit with colur blue
            ...
            if go_back is not None and attempt.current_question_index > 0:
                if answer_value != '' and "marked_for_review" not in attempt.question_status.get(str(current_question.question_id), ""):
                    attempt.question_status[str(current_question.question_id)] = "answered"
                elif answer_value == '' and "marked_for_review" not in attempt.question_status.get(str(current_question.question_id), ""):
                    attempt.question_status[str(current_question.question_id)] = "unanswered"
                attempt.current_question_index -= 1
            elif jump_to_question is not None:
                attempt.current_question_index = int(jump_to_question)
            elif go_forward is not None:
                if answer_value != '': # if the there is answer and save and next is clicked we mark it green else the previous state
                    attempt.question_status[str(current_question.question_id)] = "answered"
                if attempt.current_question_index <(quiz.questions.count()- 1):
                    attempt.current_question_index +=1
            elif marked_for_review is not None: # marked for review and next
                if answer_value == '':
                    attempt.question_status[str(current_question.question_id)] = "marked_for_review_unanswered"
                else:
                    attempt.question_status[str(current_question.question_id)] = "marked_for_review_answered"
                    
                if attempt.current_question_index <(quiz.questions.count()- 1):
                    attempt.current_question_index +=1

            elif confirm_submit is not None:
                complete_quiz(attempt,status="manual_submit")
                    
            elif clear_response is not None:
            ...
            attempt.question_start_timestamp = now()
            attempt.save(update_fields=['question_status', 'current_question_index', 'question_start_timestamp'])
            return redirect('start_quiz', quiz_id=quiz.quiz_id)
    
    else:
        form = SingleQuestionForm(quiz=quiz, user=request.user, current_question=current_question)
    ...
    context = {
        ...
        'current_question_index': attempt.current_question_index, # For user display
...
def search_quiz(request):
    ...
    return render(request,'Users/filters/search_quiz.html')


def search_resources(request):

    return render(request,'Users/filters/search_resources.html')
